...
export default function AlunosPage() {
  ...
  function cadastrar(dados: AlunoReal) {
    setAlunos((as) => [...as, dados]);
    setModal(false);
  }
  function abrirAluno(a: AlunoReal) {
    router.push(`/alunos/${alunos.indexOf(a)}`);
  }
      ...
      {buscando ? (
        resultados.length === 0 ? (
          ...
          <div className="divide-y divide-line overflow-hidden rounded-2xl border border-line bg-white">
            {resultados.map((a, i) => (
              <LinhaAluno key={a.nome + i} a={a} onClick={() => abrirAluno(a)} />
            ))}
          </div>
        )
      ) : (
...
function FormAluno({ onCancelar, onSalvar }: { onCancelar: () => void; onSalvar: (dados: AlunoReal) => void }) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        if (!nome.trim()) return;
        onSalvar({ nome: nome.trim(), tel: tel.trim() || null, turma, aula: 0, total, status: "confirmado" });
      }}
    >
      ...
      <select value={turma} onChange={(e) => setTurma(e.target.value)} className="mb-3 w-full rounded-lg border border-line bg-white px-3 py-2.5 text-sm outline-none focus:border-ink">
