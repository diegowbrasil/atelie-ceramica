...
// Referência de comportamento: demo/AtelieDemo.jsx (OficinaDetalhe). Ligado
// aos dados reais (2026-09-24) — cadastrar/editar/remover participante
// agora depende de saber se a vaga já tem uma linha real em
// `oficina_participantes` (`_id`, só presente em vaga ocupada) pra decidir
// entre `cadastrarParticipante` (INSERT) e `editarParticipante` (UPDATE).
export function OficinaDetalheClient({ oficinaInicial }: { oficinaInicial: OficinaReal }) {
  const router = useRouter();
  const [oficina, setOficina] = useState<OficinaReal>(oficinaInicial);
  ...
  // Editar não muda a rota (id continua o mesmo) — mesmo padrão de resync
  // de AlunoDetalheClient.
  useEffect(() => setOficina(oficinaInicial), [oficinaInicial]);

  async function atualizarStatusPecas(status: StatusPecas) {
    setOficina((o) => ({ ...o, statusPecas: status }));
    await editarOficina(oficina.id, { statusPecas: status });
    router.refresh();
  }

  async function salvarEdicaoOficina(dados: DadosOficina) {
    setPendente(true);
    setErroEditar(null);
    try {
      await editarOficina(oficina.id, dados);
      setModalEditar(false);
      router.refresh();
    } catch (e) {
      setErroEditar(e instanceof Error ? e.message : "Falha ao editar oficina.");
    } finally {
      setPendente(false);
    }
  }
  ...
  async function salvarParticipante(numero: number, dados: { nome: string; tipo: "individual" | "dupla"; duplaCom: string | null; pagamento: "pendente" | "pago" }) {
    ...
    try {
      if (atual?._id) {
        await editarParticipanteReal(atual._id, oficina.id, dados);
      } else {
        await cadastrarParticipanteReal(oficina.id, dados);
      }
      setModalNumero(null);
      router.refresh();
    } finally {
  ...
  async function removerParticipante(numero: number) {
    const atual = oficina.participantes.find((p) => p.numero === numero);
    if (!atual?._id) return;
    setPendente(true);
    try {
    ...
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-6 md:px-8 md:pb-10">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:flex-nowrap">
          ...
          <button onClick={() => { setErroEditar(null); setModalEditar(true); }} className="flex items-center gap-1.5 rounded-xl border border-line bg-white px-4 py-2.5 text-sm font-medium text-ink hover:bg-cream">
            Editar oficina
          </button>
      ...
      {modalNumero !== null && (
        <ModalCadastrarParticipante
          ...
          onSalvar={(dados) => salvarParticipante(modalNumero, dados)}
...
function ModalCadastrarParticipante({
  numero, participante, opcoesDupla, pendente, onClose, onSalvar, onRemover,
}: {
  numero: number;
  participante: ParticipanteReal | null;
  opcoesDupla: string[];
  pendente: boolean;
  onClose: () => void;
