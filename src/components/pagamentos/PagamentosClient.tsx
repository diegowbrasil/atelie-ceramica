...
// pagamentos (pendente + pago), diferente do mock (`pagamentosIniciais()`
// só derivava pendentes do roster) — por isso "Pendente"/"Alunos"/"Ticket
// médio" agora filtram `status === "pendente"` explicitamente em vez de
// assumir que a lista inteira já é só pendente. "Recebido" deixa de ser
// TODO(0 fixo) e vira soma real de `valor` dos pagos (hoje a maioria tem
// valor null — nunca foi informado nesta leva de dado real — mas o
// cálculo já está certo pra quando um valor de verdade existir).

interface Props {
  pagamentosIniciais: PagamentoReal[];
}

export function PagamentosClient({ pagamentosIniciais }: Props) {
  ...
  useEffect(() => setPagamentos(pagamentosIniciais), [pagamentosIniciais]);
  ...
  const totalRecebido = pagos.reduce((s, p) => s + (p.valor || 0), 0);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-6 md:px-8 md:pb-10">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard icon={Users} label="Alunos" value={pendentes.length} sub="no período" />
        <StatCard icon={Clock} label="Ticket médio" value={"R$ " + Math.round(totalPendente / (pendentes.length || 1))} />
      </div>
